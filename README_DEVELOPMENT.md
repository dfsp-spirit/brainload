...
First follow the installation instructions for the development version above if you want to run the tests. Once you have done that, continue from here.

### Running the tests locally

#### Obtaining the test data

Not all data are included with the repo, as the test data are quite large and some fake subjects are generated by copying others. The good news is that you can get all test data by running a single shell script from the repo root:
...
The script downloads all required [neuroimaging test data from Github](https://github.com/dfsp-spirit/neuroimaging_testdata) and puts it into place. If you have a properly configured local installation of FreeSurfer (with environment variable `FREESURFER_HOME` pointing to the installation directory), the script will prefer that directory for some data, but you will definitely need an internet connection to get all required data.
...
Click the icon for detailed information on the last builds.

## Packaging

This section describes how to build packaged for distribution, and how to actually distribute them by uploading to popular python package repositories.
...
Now build the latest documentation, ensure it will be included in the package that we will soon build, and make it available on Github pages so users can browse it online:

```console
cd doc        # Note that 'doc' holds the source files and tools to generate the documentation, while the content of 'docs' is what gets publishes as a Github page! Do not confuse doc with docs!
make html
```
...
cd ..
mkdir docs/v0.0.2/
cp -r doc/_build/html docs/v0.0.2/
```
Now edit the following files to update the documentation referenced in them:
- docs/index.html => should redirect to the latest version
- docs/versions.html => should list the new version

Note: If you add new source sub modules in separate directories, for the documentation to be included when running `make html`,
