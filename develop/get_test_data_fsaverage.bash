...
if [ "${MODE}" = "local_then_remote" -o "${MODE}" = "local" ]; then
    ...
    if [ -n "${FREESURFER_HOME}" ]; then
        ...
        echo "Copying FreeSurfer license file from '${FREESURFER_HOME}' to '${BRAINLOAD_TEST_DATA_DIR_FSAVERAGE}'."
        for LFILE in LICENSE; do
            cp "${FREESURFER_HOME}/${LFILE}" "${BRAINLOAD_TEST_DATA_DIR_FSAVERAGE_SURF}" && echo " * ${LFILE}"
        done

        # Copy only the data we need for testing. The whole fsaverage subject is vast.
        echo "Copying fsaverage data from '${FD}' to '${BRAINLOAD_TEST_DATA_DIR_FSAVERAGE_SURF}'."
        for TFILE in lh.inflated lh.pial lh.sphere lh.white rh.inflated rh.pial rh.sphere rh.white; do
            cp "${FD}/${TFILE}" "${BRAINLOAD_TEST_DATA_DIR_FSAVERAGE_SURF}" && echo " * ${TFILE}"
        done
...
if [ "${MODE}" = "local_then_remote" -a ${DONE_ALREADY} = "NO" -o "${MODE}" = "remote" ]; then
    ...
    cd "${BRAINLOAD_TEST_DATA_DIR}" && wget "${REMOTE_ZIP_URL}" && unzip -f -o ${ARCHIVE_NAME} && rm ${ARCHIVE_NAME} && echo "OK."
fi
